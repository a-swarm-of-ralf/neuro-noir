...
def _(mo):
    ...
    Entity type attributes cannot use protected names that are already used by Graphiti’s core EntityNode class:
...
def _(mo):
    ...
    For each chunk ingest it into the database by calling the `neuro_noir.core.database.add_episode` function. Comment-in the `await _()` line in the code below.

    ///

    /// attention | First Time Error

    Running the first time sometimes generates an error. Deleting the database with the button in the settings section will help and should fix the error.
    ///
    """)
    return
...
async def _(
    ...
    graphiti = await connect_graphiti(app.cfg)
...
@app.cell
def _():
    # await graphiti.search("What do we know about Sherlock Holmes?"):
    return
