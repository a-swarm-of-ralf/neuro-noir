...
def _(mo):
    ...
    This is Part 1 of a two-part workshop. In this section, you will focus on extracting information from text and building a knowledge graph to help solve a murder mystery.

    1. **Test Settings**: Verify your connections to the database and language model.
    2. **Select a Case**: Choose one of the four cases to use in your workshop.
    3. **Chunk the Text**: Split the story into manageable segments for processing.
    4. **Extract Triples**: Identify subject–predicate–object relationships from the text.
    5. **Resolve Entities**: Link extracted mentions to consistent entity representations.
    6. **Coreference Resolution**: Detect and merge references that point to the same real-world entity.
    7. **Disambiguate Entities**: Distinguish between entities with similar or identical names.

    After completing these steps, the workshop continues in Part 2, where you will design and build an agent that consumes the generated knowledge graph.
...
def _(mo):
    ...
    In this section, you will learn how to identify and merge different references that point to the same real-world person, place, or object.

    /// details | What Are Coreferences and Why Do They Matter?
        type: info
    When people write or speak, they often refer to the same thing in different ways. For example, a character might be mentioned by name in one sentence, by a pronoun in another (“he” or “she”), and by a description elsewhere (“the suspect” or “the neighbour”). These different expressions are called *coreferences* when they all point to the same underlying entity. If these references are not resolved, the knowledge graph will treat them as separate nodes, which leads to fragmented information and makes reasoning unreliable.
    ///
    /// details | How Coreference Resolution Works
        type: info
    To resolve coreferences, you will process the extracted entities one by one. For each entity, you will search for potentially related entities using vector similarity based on embeddings. You will then use an LLM through DSPy to evaluate which of these candidates refer to the same real-world entity. Next, you will use the LLM again to merge each confirmed group of references into a single, unified entity representation. Finally, you will apply these merges to your Neo4j graph so that all relevant relationships point to the consolidated node.
    ///
    /// admonition | Exercise 7.

    Implement a coreference resolution pipeline that finds similar entities, selects merge candidates using DSPy, consolidates them into unified entities, and updates the Neo4j graph accordingly.
    ///
    """)
    return
...
def _(Entity, app, dspy, mo):
    ...
    class CoreferenceResolver(dspy.Signature):
        """
        Your task is, given a target entity, to resolve corefrences from a list of candidate entities.

        Instructions
        ------------
        1. First examine the target entity carefully.
        2. Next for each candidate consider if it is a coreference and should be merged. Consider 
           the ones that should be merged the selected candidates. If non or de candidates are 
           coreference don't merge and add a note to the explanation that no coreferences were found.
        3. Merge the target with the selected candidates by going over each field:
            - id: The id should remain the candidate id
            - name: From the target and selected candidates select the anme that best represents the 
              entity. The rest of the names should be put into the aliases list.
            - aliases: Combine the aliases from target and selected candidates. Also add the names that
              were not selected as the merged name.
            - category: The category is not expected to differ, but if it does select the best one and
              explain your choice in the explaintion later.
            - description: Combine the description into a single coherent description.
            - explanation: Join the explanations of the target and selected candidates, then add an 
              explanation of why you merged these candidates and the choices you made during the merge.
        4. Finaly list all the ids of the selected candidates that were merged with the target into the
           'merged_candidate_ids' field.
        """

        target_entity: str = dspy.InputField(desc="The target entity to resolev corefrences for.", json_schema=ENTITY_SCHEMA)
        entity_candidates: list = dspy.InputField(
            desc="JSON array of candidate entities that posibly refer to the same \"real-life\" target entity.",
...
def _(mo):
    ...
    In this section, you will learn how to distinguish between different real-world entities that share the same or very similar names.

    /// details | What Is Entity Disambiguation
        type: info

    In many texts, different people, places, or objects can have identical or nearly identical names. For example, two characters may share the same surname, or a location name may refer to multiple places. Without disambiguation, a knowledge graph may incorrectly merge separate individuals into one node, causing false connections and misleading conclusions. Entity disambiguation ensures that each node represents exactly one real-world entity, preserving the accuracy of the investigation.
    ///

    /// details | How Disambiguation Differs from Coreference Resolution
        type: info

    Although both tasks deal with resolving ambiguity, they address opposite problems. Coreference resolution focuses on finding multiple references that point to the same real-world entity and merging them into one node. Disambiguation, on the other hand, focuses on detecting when a single node actually represents multiple different entities and separating it into distinct nodes. In other words, coreference resolution reduces duplication, while disambiguation prevents over-merging.
    ///
    /// details | How Entity Disambiguation Works
        type: info

    To disambiguate entities, you will examine groups of entities with similar names or attributes. For each group, you will collect contextual information such as related events, relationships, locations, and timelines from the graph. You will then use an LLM through DSPy to analyze this context and determine whether the entities represent the same individual or distinct ones. When necessary, you will split incorrectly merged nodes into separate entities and reassign their relationships. Finally, you will update the Neo4j graph so that each entity is uniquely and correctly represented.
    ///
    /// admonition | Exercise 8.

    Implement an entity disambiguation pipeline that analyzes contextual evidence, separates overlapping identities, and updates the Neo4j graph to ensure each node represents a distinct real-world entity.
    ///
    """)
    return
...
def _():
    ...
    nest_asyncio.apply()
    app = Application()
    return BaseModel, Entity, Field, Optional, app, dspy, list_datasets, mo
